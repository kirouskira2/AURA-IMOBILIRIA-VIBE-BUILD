        ...
        >
          ...
          <div className={`flex w-full items-center justify-between h-[80px] px-6 shrink-0 z-20 relative mx-auto`}>
            ...
            <div className="ml-auto flex h-full items-center gap-4">
              ...
              <PulsatingButton 
                onClick={() => window.open('https://wa.me/5581973380824', '_blank', 'noopener,noreferrer')}
                className="hidden md:flex bg-black text-white font-medium transition-transform hover:scale-105" 
                pulseColor="rgba(0,0,0,0.5)"
              >
                WhatsApp
              </PulsatingButton>
              ...
              >
                <div className="w-full h-full mx-auto flex gap-4">
                  <div className="flex flex-1 gap-4 h-full">
                    <Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`flex-1 border rounded-3xl overflow-hidden relative group/card h-full ${isDarkNav ? 'border-white/5 bg-black/40' : 'border-black/5 bg-white/40'}`}>
                  ...
                  <div className={`w-[280px] shrink-0 flex flex-col py-6 px-6 rounded-3xl border ${isDarkNav ? 'bg-[#151515] border-white/5' : 'bg-zinc-50 border-black/5'}`}>
                    ...
                    <ul className="flex flex-col gap-4 mb-auto">
                      <li><Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`text-sm transition-colors flex items-center gap-3 ${isDarkNav ? 'text-zinc-400 hover:text-white' : 'text-zinc-600 hover:text-black'}`}><span className={`w-1.5 h-1.5 rounded-full ${isDarkNav ? 'bg-white/30' : 'bg-black/30'}`}></span> Reserva Cidade Jardim</Link></li>
                      <li><Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`text-sm transition-colors flex items-center gap-3 ${isDarkNav ? 'text-zinc-400 hover:text-white' : 'text-zinc-600 hover:text-black'}`}><span className={`w-1.5 h-1.5 rounded-full ${isDarkNav ? 'bg-white/30' : 'bg-black/30'}`}></span> Fasano Residences SP</Link></li>
                      <li><Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`text-sm transition-colors flex items-center gap-3 ${isDarkNav ? 'text-zinc-400 hover:text-white' : 'text-zinc-600 hover:text-black'}`}><span className={`w-1.5 h-1.5 rounded-full ${isDarkNav ? 'bg-white/30' : 'bg-black/30'}`}></span> Fazenda Boa Vista</Link></li>
                      <li><Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`text-sm transition-colors flex items-center gap-3 ${isDarkNav ? 'text-zinc-400 hover:text-white' : 'text-zinc-600 hover:text-black'}`}><span className={`w-1.5 h-1.5 rounded-full ${isDarkNav ? 'bg-white/30' : 'bg-black/30'}`}></span> CJ Shops</Link></li>
                    </ul>
                    <Link href="#imoveis" onClick={() => setIsCollectionsHovered(false)} className={`mt-6 font-semibold py-3 px-6 rounded-2xl inline-flex items-center justify-center gap-2 transition-colors text-sm border ${isDarkNav ? 'bg-zinc-800 hover:bg-zinc-700 text-white border-white/10' : 'bg-zinc-200 hover:bg-zinc-300 text-black border-black/5'}`}>
